        ...
        if labeled_loader is not None and epoch >= phase1_end:
            ...
            for batch in labeled_loader:
                ...
                try:
                    ...
                    with autocast():
                        out = model(x)
                        # Usa scalar_labels diretamente como peso (sem atenuar pelo GMM).
                        # A versão anterior multiplicava por sf_conf (pseudo_label_confidence)
                        # do ScalarFieldGMM. Quando o GMM é mal calibrado (w_anomal≈0.77),
                        # sf_conf≈0 para pontos de rachadura real, silenciando o Push-Pull.
                        loss = push_pull(
...
# ============================================================================
# SOLUÇÃO A — GEOMETRIC MULTI-FEATURE GMM
# ============================================================================

class GeometricMultiGMM:
    """
    GMM multidimensional ajustado sobre features geométricas por ponto.

    Features: curvatura (col 11), densidade (col 12), variância (col 13).
    Em superfícies com rachadura, curvatura é alta (borda de descontinuidade),
    densidade pode ser menor (interior do void) e variância é maior (rugosidade).
    Juntos formam uma distribuição bimodal no espaço 3D que separa crack de normal.

    Bimodalidade avaliada via diferença de BIC (GMM-2 vs GMM-1).
    Unimodal → anomaly_probability() retorna 0.5 neutro (sem penalidade).

    Referência: uso de multi-feature GMM para detecção de anomalias em nuvens
    de pontos — adaptado de Han et al. (2022), Point Cloud Anomaly Detection.
    """

    # Índices das features geométricas no vetor completo (dim=15)
    GEOM_COLS = [11, 12, 13]   # curvatura, densidade, variância

    def __init__(self, features_raw: np.ndarray, n_components: int = 2):
        """
        features_raw : (N, D) — features completas (D≥14), ou (N, 3) já fatiado.
        Se D > 3, extrai automaticamente GEOM_COLS.
        """
        arr = np.asarray(features_raw, dtype=np.float32)
        if arr.ndim == 1:
            arr = arr.reshape(1, -1)
        if arr.shape[1] > 3:
            arr = arr[:, self.GEOM_COLS]
        self.X = arr                      # (N, 3) — [curv, dens, var]
    ...
    def fit(self) -> 'GeometricMultiGMM':
        from sklearn.mixture import GaussianMixture
        from sklearn.preprocessing import RobustScaler

        X = self.X
        if len(X) < 30 or X.std(axis=0).mean() < 1e-8:
            self._modality = 'unimodal'
            self._fitted   = True
            return self

        # Normalização robusta — resiste a outliers de superfície ruidosa
        scaler = RobustScaler()
        ...
        try:
            ...
            gmm2 = GaussianMixture(n_components=self.n_components,
                                   covariance_type='full', max_iter=300,
                                   random_state=42, n_init=5)
            gmm1.fit(Xs)
            gmm2.fit(Xs)

            bic_delta  = gmm1.bic(Xs) - gmm2.bic(Xs)
            means      = gmm2.means_                         # (2, 3)
            covs       = gmm2.covariances_                   # (2, 3, 3)
            avg_std    = (np.sqrt(np.trace(covs[0])) +
                          np.sqrt(np.trace(covs[1]))) / 2.0
            separation = np.linalg.norm(means[0] - means[1]) / (avg_std + 1e-8)

            if bic_delta > 10 and separation > 0.5:
                self._modality = 'bimodal'
                self._gmm      = gmm2
                # Componente de rachadura: maior curvatura (col 0 do espaço norm.)
                # Curvatura alta é o indicador mais robusto de borda de rachadura.
                # Densidade e variância são usadas como desempate.
                crack_score      = means[:, 0] - 0.3 * means[:, 1] + 0.3 * means[:, 2]
                self._crack_idx  = int(np.argmax(crack_score))
            else:
                self._modality = 'unimodal'
                self._gmm      = None

        except Exception as e:
            log.warning(f"GeometricMultiGMM: GMM falhou ({e})")
            self._modality = 'unimodal'
            self._gmm      = None
    ...
    def anomaly_probability(self) -> np.ndarray:
        """P(rachadura | [curv, dens, var][i]) por ponto. Unimodal → 0.5 neutro."""
        if not self._fitted:
            self.fit()
        if self._gmm is None:
            return np.full(len(self.X), 0.5, dtype=np.float32)
...
def compute_anomaly_scores(model: TeacherStudentModel,
    ...
    """
    Score composto por ponto — Solução A (Multi-Feature GMM):

      score(p) = 0.55 · sf_score(p)    [ScalarFieldGMM    — sinal primário]
               + 0.30 · geom_score(p)  [GeometricMultiGMM — curvatura/dens/var]
               + 0.15 · distill_norm(p)[Teacher-Student    — prior geométrico]

    Normalização do componente de distilação: percentil 2–98 (robusto a outliers
    de superfície ruidosa que inflavam o intervalo min-max para o ruído).

    sf_score  : P(rachadura | scalar_field) via GMM 1D bimodal (col 9).
    geom_score: P(rachadura | curvatura, densidade, variância) via GMM 3D (cols 11-13).
    distill   : discrepância Teacher-Student normalizada por percentil.

    Se memory_bank=None: distill_norm = distilação pura (sem banco de memória).
    """
    model.eval()
    results = []
    ...
    for d in data_list:
        ...
        # ── Distilação Teacher-Student (prior geométrico fraco) ───────────────
        ...
        # ── ScalarFieldGMM (col 9 — sinal primário, provado discriminativo) ──
        sf_raw   = d['features'][:, 9]
        sf_gmm   = ScalarFieldGMM(sf_raw).fit()
        sf_score = sf_gmm.anomaly_probability()              # (N,) ∈ [0,1]

        # ── GeometricMultiGMM (cols 11,12,13 — curvatura, densidade, variância)
        geom_gmm   = GeometricMultiGMM(d['features']).fit()
        geom_score = geom_gmm.anomaly_probability()          # (N,) ∈ [0,1]

        # ── Fusão final (Solution A) ──────────────────────────────────────────
        score = 0.55 * sf_score + 0.30 * geom_score + 0.15 * distill_component
...
def main():
    ...
    # Cada execução salva em subpasta própria com data+hora+minuto+segundo
    run_results = os.path.join(RESULTS, f'run_{ts}')
    run_vis     = os.path.join(VIS_PATH, f'run_{ts}')
    run_ply     = os.path.join(PLY_PATH, f'run_{ts}')

    for p in [run_results, run_vis, run_ply, MODELS, LOGS_PATH]:
    ...
    # ── 7. Threshold GMM ──────────────────────────────────────────────────────
    # [SOLUÇÃO A — gate soft movido para ANTES do threshold]
    # O gate soft modula o score contínuo; deve ser aplicado ANTES da
    # binarização. Antes (v2 original) era aplicado DEPOIS do apply_threshold,
    # o que não alterava pred_labels e era redundante com o sf_score já
    # embutido na fusão 0.55·sf + 0.30·geom + 0.15·distill.
    #
    # [ANTES — v2 original: gate soft DEPOIS do threshold — removido]
    ...
    #     if sf_raw is None: continue
    #     sf_gmm = ScalarFieldGMM(sf_raw).fit()
    #     r['score'] = r['score'] * sf_gmm.soft_weights()
    #
    # [NOVO — Solução A: score já inclui sf_score; gate soft antes do threshold]
    log.info("Solução A: score = 0.55·sf_gmm + 0.30·geom_gmm + 0.15·distill")
    thr, gmm_info = fit_gmm_threshold(results, normal_results=normal_ref)
    ...
    comp_path = os.path.join(run_results, f'comparacao_thresholds_{ts}.csv')
    ...
    relatorio_path = os.path.join(run_results, f'relatorio_severidade_{ts}.csv')
    ...
    nuvens_path = os.path.join(run_results, f'severidade_por_nuvem_{ts}.csv')
    ...
    try:
        save_results(metrics, results, history, thr_f1, gmm_info, run_results, ts=ts)
    except Exception as e:
    ...
    sev_path = os.path.join(run_results, f'relatorio_severidade_{ts}.csv')
    ...
    # ── Resumo ────────────────────────────────────────────────────────────────
    print("\n" + "="*70)
    print("  RESULTADOS FINAIS — PROFESSOR-ALUNO v2")
    print("="*70)
    if metrics:
